...
```
где `4444:4444` - `порт_локальной_машины:порт_внутри_контейнера`, параметром `HUB_HOST=selenium-hub` мы указываем на имя нашего хаба `--name selenium-hub`, а `-d` - позволяет нам зупустить еонтейнер в фоновом режиме.
...
3. Для дальнейшего удобства запуска отчётов, Allure нужно добавить в системную переменную $PATH:
    - Выаолняем в терминале команду: `nano ~/.bashrc `;
...
```
где `--alluredir=~/stepik_traineeship/allure/` - папка где храняться отчёты по тестам.
